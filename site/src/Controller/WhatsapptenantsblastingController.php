 ...
 * @since  1.6.0
 */
class WhatsapptenantsblastingController extends BaseController
{
	/**
	...
	public function edit()
	{
		...
		// Get the model.
		$model = $this->getModel('Whatsapptenantsblasting', 'Site');
	...
	public function publish()
	{
		...
		if ($user->authorise('core.edit', 'com_dt_whatsapp_tenants_blastings') || $user->authorise('core.edit.state', 'com_dt_whatsapp_tenants_blastings'))
		{
			...
			if (!$item)
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_dt_whatsapp_tenants_blastings&view=whatsapptenantsblastings', false));
			}
			else
			{
	...
	public function checkin()
	{
		...
		if ($user->authorise('core.manage', 'com_dt_whatsapp_tenants_blastings') || $item->checked_out == $user->id) { 
			...
			if ($return === false)
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_dt_whatsapp_tenants_blastings&view=whatsapptenantsblasting' . '&id=' . $id, false), $message, 'error');
				return false;
			}
			else
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_dt_whatsapp_tenants_blastings&view=whatsapptenantsblasting' . '&id=' . $id, false), $message);
				return true;
			}
		}
		else
	...
	public function remove()
	{
		...
		if ($user->authorise('core.delete', 'com_dt_whatsapp_tenants_blastings'))
		{
			...
			else
			{
				...
				$this->app->redirect(Route::_('index.php?option=com_dt_whatsapp_tenants_blastings&view=whatsapptenantsblastings', false));
			}
