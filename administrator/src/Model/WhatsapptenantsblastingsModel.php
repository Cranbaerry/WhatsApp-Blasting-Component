...
class WhatsapptenantsblastingsModel extends ListModel
{
	...
	*/
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.status LIKE ' . $search . '  OR #__dt_whatsapp_tenants_keywords_4169679.name LIKE ' . $search . '  OR #__dt_whatsapp_tenants_templates_4168267.name LIKE ' . $search . '  OR  a.mode LIKE ' . $search . '  OR  a.scheduled_time LIKE ' . $search . ' )');
			}
		}
	...
	public function getItems()
	{
		...
		{
					$oneItem->type = !empty($oneItem->type) ? Text::_('COM_DT_WHATSAPP_TENANTS_BLASTINGS_WHATSAPPTENANTSBLASTINGS_TYPE_OPTION_' . preg_replace('/[^A-Za-z0-9\_-]/', '',strtoupper(str_replace(' ', '_',$oneItem->type)))) : '';
