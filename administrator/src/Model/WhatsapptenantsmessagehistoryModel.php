...
/**
 * Whatsapptenantsmessagehistory model.
 *
 * @since  1.0.0
 */
class WhatsapptenantsmessagehistoryModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_dt_whatsapp_tenants_blastings.whatsapptenantsmessagehistory';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
								'com_dt_whatsapp_tenants_blastings.whatsapptenantsmessagehistory', 
								'whatsapptenantsmessagehistory',
								array(
									'control' => 'jform',
									'load_data' => $loadData 
								)
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_dt_whatsapp_tenants_blastings.edit.whatsapptenantsmessagehistory.data', array());
	...
	/**
	 * Method to duplicate an Whatsapptenantsmessagehistory
	 *
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
			if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser())
			{
				...
				if (!empty($table->phone_number_id))
				{
					if (is_array($table->phone_number_id))
					{
						$table->phone_number_id = implode(',', $table->phone_number_id);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__dt_whatsapp_tenants_message_history');
				$max             = $db->loadResult();
